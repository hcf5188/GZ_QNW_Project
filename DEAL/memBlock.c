...
uint8_t Mem_free(void **ptr)
{
	...
	*ptr = NULL;
	if(ptr_free == NULL)
		return 1;
	ptrBuff = (uint16_t*)ptr_free ;
	ptrBuff = ptrBuff - 1;
	switch(*ptrBuff)
	{
		case MEM_16B_COL: 
			allMemState.memUsedNum1 --;
			memset(ptrBuff,0,MEM_16B_COL);
			return OSMemPut(pMemBuf_16B,ptrBuff);
		case MEM_32B_COL: 
			allMemState.memUsedNum2 --;
			memset(ptrBuff,0,MEM_32B_COL);
			return OSMemPut(pMemBuf_32B,ptrBuff);
		case MEM_64B_COL: 
			allMemState.memUsedNum3 --;
			memset(ptrBuff,0,MEM_64B_COL);
			return OSMemPut(pMemBuf_64B,ptrBuff);
		case MEM_128B_COL: 
			allMemState.memUsedNum4 --;
			memset(ptrBuff,0,MEM_128B_COL);
			return OSMemPut(pMemBuf_128B,ptrBuff);
		case MEM_256B_COL: 
			allMemState.memUsedNum5 --;
			memset(ptrBuff,0,MEM_256B_COL);
			return OSMemPut(pMemBuf_256B,ptrBuff);
		case MEM_512B_COL: 
			allMemState.memUsedNum6 --;
			memset(ptrBuff,0,MEM_512B_COL);
			return OSMemPut(pMemBuf_512B,ptrBuff);
		case MEM_1KB_COL: 
			allMemState.memUsedNum7 --;
			memset(ptrBuff,0,MEM_1KB_COL);
			return OSMemPut(pMemBuf_1KB,ptrBuff);
		default :
			return 255;
	}
}
